...
void App::Start() {
  ...
  m_Chest =
      std::make_shared<Character>(GA_RESOURCE_DIR "/Image/Character/chest.png");
  m_Chest->SetZIndex(5);
  ...
  beeImages.reserve(2);
  for (int i = 0; i < 2; ++i) {
    beeImages.emplace_back(GA_RESOURCE_DIR "/Image/Character/bee_" +
  ...
  for (int i = 0; i < 3; ++i) {
    m_Doors.push_back(std::make_shared<Character>(
  ...
  std::vector<std::string> countdownPaths = {
  ...
  m_Ball = std::make_shared<AnimatedCharacter>(countdownPaths);
  m_Ball->SetPosition({-130.0f, -175.0f});
